...
class Discriminator:
    ...
    def _build(self):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            self.tag_weight = tf.get_variable(name="tag_weight", shape=[22, self.tag_dim], 
                                                 initializer=tf.contrib.layers.xavier_initializer())
            self.tag_bias = tf.get_variable(name="tag_bias", shape=[self.tag_dim],
...
#             output4 = tf.nn.leaky_relu(output4_)
            ...
            tag_vec = tf.reshape(
                tf.nn.leaky_relu(tf.nn.xw_plus_b(tag_oh, self.tag_weight, self.tag_bias)),
                [-1, 1, 1, self.tag_dim])
